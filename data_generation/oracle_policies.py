...
class Goal2OracleController:
    ...
    def _success_action(
        ...
        if task_id == "E2_L5_DualPatrol":
            return self._dual_patrol_action(agent_pos, agent_vel)
    ...
    def _dual_patrol_action(
        ...
        if distance_xy(agent_pos, target) <= float(zone["radius"]):
            self.state.patrol_dwell_steps += 1
            if self.state.patrol_dwell_steps >= self.patrol_switch_dwell_steps:
                self.state.current_patrol_target = "B" if current == "A" else "A"
                self.state.patrol_dwell_steps = 0
                current = self.state.current_patrol_target
                zone = self.resolved_zones[current]
                target = np.asarray(zone["center"], dtype=float)
        else:
            self.state.patrol_dwell_steps = 0
    ...
    def _full_mission_action(
        ...
        if self.state.phase == "C_patrol":
            ...
            return self._compose_action(
                ...
                agent_vel,
                target,
                speed_limit=self.fast_threshold * 0.78,
                use_hazard_avoidance=True,
                use_vase_avoidance=False,
            )
    ...
    def _forward_command(
        ...
        current_speed = float(np.linalg.norm(agent_vel))
        terminal_docking = target_distance < 0.48
        final_docking = target_distance < 0.65
